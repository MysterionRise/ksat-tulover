...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<f32>("400,500", ','), Some((400., 500.)));
}
...
fn render(
    ...
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + column] = match complex_square_add_loop(point, limit) {
                None => 0,
...
fn main() {
    ...
    if args.len() != 6 {
        writeln!(
            std::io::stderr(),
            "Usage: mandelbrot FILE LIMIT PIXELS UPPERLEFT LOWERRIGHT"
        )
        .unwrap();
        writeln!(
            std::io::stderr(),
            "Example: {} mandel.png 300 1600x1200 -1.20,0.35 -1,0.20",
            args[0]
        )
        .unwrap();
        std::process::exit(1);
    ...
    println!("Number of CPUs detected: {}", cpus);
    let threads = cpus * 4;
        ...
        crossbeam::scope(|spawner| {
            for (i, band) in bands.into_iter().enumerate() {
                ...
                spawner.spawn(move |_| {
                    render(band, band_bounds, band_upper_left, band_lower_right, limit);
                });
            }
        });
    }
    ...
    println!("Execution time: {} milliseconds", now.elapsed().as_millis());
}
